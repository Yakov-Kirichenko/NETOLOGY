...
from pyspark.sql.functions import col,lead,max
...
df1=df.select(df.iso_code,df.location,((df.total_cases*100) / df.population).cast('decimal(12,2)').alias('perscent'))\
...
.filter(~col('iso_code').startswith('OWID'))\
